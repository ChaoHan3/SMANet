...
class SMANet(nn.Module):
    def __init__(self, num_in_ch=3, num_out_ch=3, num_feat=64, num_block=12, upscale=2, res_scale=1.0):
...
class ResidualBlock(nn.Module):  # 实现了残差块的功能
    def __init__(self, num_feat=64, res_scale=1,idynamic_ffn_expansion_factor=2,input_resolution=None,num_head=8, chsa_head_ratio=0.25, window_size=8, shift=0, head_dim=None, qkv_bias=True, mv_ver=1,
                 hidden_ratio=2.0, act_layer=nn.GELU, attn_drop=0.0, proj_drop=0.0,
                 drop_path=0.0, helper=True,
                 mv_act=nn.LeakyReLU, exp_factor=1.2, expand_groups=4):  # 特征数量64，res_scale残差比例，默认为1，残差缩放比例的作用是在残差网络中调节残差的重要性，提高模型的性能和训练稳定性。
        ...
        self.baseblock7 = Baseblock7(num_feat, ffn_expansion_factor=idynamic_ffn_expansion_factor, bias=False, input_resolution=input_resolution)
